...
//                if(i == 1)
//					__AND_LOGD("before dbLevel %d  %d ",mmh->participants[i].dbLevel,(++myTime)*20/1000);
                piframes[pnum++] = pi;
                int j = 0;
        ...
        if(mmh->oframes[mmh->wIdx].rwflag == FS_WRITEABLE)
        {
			...
			__AND_LOGD("after dbLevel all: %d  1:%d  2:%d  3:%d  4:%d  5:%d  6:%d  time:%lf\n", MCheckLevel(mmh->oframes[mmh->wIdx-1].buf, FRAME_LEN),mmh->participants[0].dbLevel,mmh->participants[1].dbLevel,mmh->participants[2].dbLevel,mmh->participants[3].dbLevel,mmh->participants[4].dbLevel,mmh->participants[5].dbLevel,(double)(++myTime)*20/1000);
