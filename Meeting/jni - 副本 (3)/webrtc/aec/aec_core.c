...
static void ProcessBlock(AecCore* aec) {
  ...
  if (aec->noiseEstCtr < noiseInitBlocks) {
    ...
    aec->noisePow = aec->dInitMinPow;
//	base_message("aec->noisePow if %lf",*(aec->noisePow));
  } else {
  ...
  WebRtcAec_OverdriveAndSuppress(aec, hNl, hNlFb, efw);
	...
	{////ƒ‹¡øº∆À„
	    ...
	    y = -efw[1][i];
			...
			energysum += sqrtf((x/1000)*(x/1000)) + sqrtf((y/1000)*(y/1000));
  	}
  	energysum = (float)log10(energysum+1);
  	setAsyncResultF(energysum);
	}
...
//	base_message("noisePow[%d] = %f",i,noisePow[i]);
//	base_message("cosf(tmp) = %f sinf(tmp) = %f ",cosf(tmp),sinf(tmp));
  }
  u[PART_LEN][1] = 0;
float max = 0;
  for (i = 0; i < PART_LEN1; i++) {
...
//	efw[0][i] = 1000000;
//	efw[1][i] = -1000000;
//	base_message("tmp = %f ",tmp);
//	base_message("u[i][0] = %f u[i][1] = %f",u[i][0],u[i][1]);

//	base_message("efw %f ",max);
  }
