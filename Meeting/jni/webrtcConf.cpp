	...
	while(gMMixer.running)
	{
		...
		for(i = 0; i < MAX_PARTICIPANT; i++)
		{
				...
				&& pi->rwflag == FS_READABLE)
			{
				...
				MHighFilter(mmh->participants[i].hf, pi->buf, FRAME_LEN);
		...
		*/
		...
		if(mmh->policy && pnum > 1)
		{
			int devol = 0;
			int overf = 0;
			int mgain[MAX_PARTICIPANT] = {0, 6, 10, 12, 14, 16};
		...
		//如果未及时取走语音，跳过当帧混音
		//webrtc_message("pnum=%d [%d]", pnum, mmh->tick);
		if(pnum >= 1)
		{
			if(mmh->oframes[mmh->wIdx].rwflag == FS_WRITEABLE)
			{
				...
				for(i = 0; i < pnum; i++)
				{
					//TODO:计算gain
					int gain = 1;
					MixTo(mmh->oframes[mmh->wIdx].buf, piframes[i]->buf, FRAME_LEN, gain);
		...
		mmh->tick ++;
		int st = mmh->tick*FRAME_TMS + oriT - MGetCurTime(0);
		if(st > 0)
			MSleepMs(st);
	}
}
	...
	if(mmh)
	{
		...
		for(i = 0; i < MAX_PARTICIPANT; i++)
		{
			if(mmh->participants[i].id == DEAD_V)
			{
				...
				mmh->participants[i].hf = MAllocHighFilter();
