...
{
	void * hf = malloc(sizeof(struct FilterState));
	InitializeFilter((struct FilterState*)hf);
	return hf;
}
...
{
	if(handle)
		Filter((struct FilterState*)handle, (int16_t*)buf, len/2);
}
...
{
	int i;
	signed short* po = (signed short*)mbuf;
	signed short* pi = (signed short*)pbuf;
	int l = len/2;
	for(i = 0; i < l; i++)
	{
		int tmp = static_cast<int32_t>(po[i]) + static_cast<int32_t>(pi[i])*gain;
		if (tmp < -0x8000)
		{
	 		tmp = -0x8000;
	  }
